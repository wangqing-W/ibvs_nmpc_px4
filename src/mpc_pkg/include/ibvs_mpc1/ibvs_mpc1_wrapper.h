...
// here "-4" means we need to eliminate the cost of orientation(q_w, q_x, q_y, q_z)
static constexpr int kStateSize = ACADO_NX;   // number of states 10
static constexpr int kRefSize = ACADO_NY;     // number of reference states 14
static constexpr int kEndRefSize = ACADO_NYN; // number of end reference states 11
static constexpr int kInputSize = ACADO_NU;   // number of inputs 4
static constexpr int kCostSize = ACADO_NY - ACADO_NU; // number of state costs 11
static constexpr int kOdSize = ACADO_NOD;     // number of online data 0
...
class IbvsMpc1Wrapper
{
 public:
  ...
  bool setLimits(T min_thrust, T max_thrust, T max_quad_xyz,
  ...
  bool setReferenceVelocity(
    const Eigen::Ref<const Eigen::Matrix<T, kStateSize, 1>> state);
  bool setVelTrajectory(
 ...
 private:
  ...
  Eigen::Map<Eigen::Matrix<float, 3, kSamples, Eigen::ColMajor>>
    acado_affine_lower_bounds_{acadoVariables.lbAValues};

  Eigen::Map<Eigen::Matrix<float, 3, kSamples, Eigen::ColMajor>>
    acado_affine_upper_bounds_{acadoVariables.ubAValues};
