...
*
* [2] C. Akinlar and C. Topal, �EDCircles: Realtime Circle Detection by Edge Drawing (ED),?*     IEEE Int'l Conf. on Acoustics,  Speech, and Signal Processing (ICASSP)}, March 2012.
**************************************************************************************************************/
...
struct mCircle {
	...
	void operator = (const mCircle circle){
...
struct mEllipse {
	...
	void operator = (const mEllipse ellipse){
        this->center = ellipse.center;
		this->axes = ellipse.axes;
        this->theta = ellipse.theta;
	}
};
...
private:
	...
	void DetectArcs(std::vector<LineSegment> lines, int maxsegnum);
	...
	static void CalculatenPoints(double r, int* noPoints);
