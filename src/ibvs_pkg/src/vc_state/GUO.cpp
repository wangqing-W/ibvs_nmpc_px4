...
Mat Lvl(Mat p2s,                    // Points of the actual image in the sphere
        Mat p2,                     // Points of the actual image in the image
        vector<float> depthVec,  // Actual depth image
...
)
{
        ...
        Mat psi, psj, pi, pj;                          // Temporal points for calculation
        double s;
        cout << (params.control == 1 ? "Control 1: 1/dist" : "Control 2: dist") << endl;
        for (int i = 0; i < n; i++)
        {
                pi = p2.row(distances[i].i);
                pj = p2.row(distances[i].j);
                psi = p2s.row(distances[i].i);
                psj = p2s.row(distances[i].j);
                // cout << "[INFO] getting depth" << endl;
                float d_i = depthVec[distances[i].i];
                float d_j = depthVec[distances[i].j];
                // cout << "[INFO] depth_i and j:" << d_i << ", " << d_j << endl;
                if (params.control == 1)
                {
                ...
                // cout << "[INFO] cal lvl" << endl;
                temp = s * ( (psi * ortoProj(psj)/d_i) + (psj * ortoProj(psi))/d_j );
                temp.copyTo(L.row(i));
        }
        temp.release();
        psi.release();
...
int sgn(double qe2_w){
        if(qe2_w > 0) return 1;
        else return -1;
}

int GUO(Mat img,                                      // Color Image to be processed
        vector<float> depthVec,                                    // Depth Image to be processed
...
)
{
        ...
        Mat p1s, p2s, p23D, Lo, U, U_temp, L, q_e2;
        ...
        float kz = state.params.gainw;

        // Get interaction matrix and error vector with distances
        L = Lvl(p2s, matching_result.p2, depthVec, distancias, state.params);
