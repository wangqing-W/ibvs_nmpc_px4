...
void vc_state::load(const ros::NodeHandle &nh)
{
  ...
  this->params.K_depth = cv::Mat(3, 3, CV_64F, double(0));
  ...
  if (nh.hasParam("camera_depth_intrinsic_parameters"))
  {
    nh.getParam("camera_depth_intrinsic_parameters", kConfig);
    if (kConfig.getType() == XmlRpc::XmlRpcValue::TypeArray)
      for (int i = 0; i < 9; i++)
      {
        std::ostringstream ostr;
...
std::pair<Eigen::VectorXd, float> vc_state::update()
{
  ...
  // else
  // {
  Eigen::Vector3d deltaB(this->Vx * this->dt, this->Vy * this->dt, this->Vz * this->dt);
  Eigen::Vector3d deltaW;
  deltaW = this->quad * deltaB;
  this->X = this->X + this->Kv * deltaW.x();
  this->Y = this->Y + this->Kv * deltaW.y();
  this->Z = this->Z + this->Kv * deltaW.z();
  // }
