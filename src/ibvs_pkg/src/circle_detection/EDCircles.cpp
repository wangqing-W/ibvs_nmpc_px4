#include "circle_detection/EDCircles.h"
...
#define CIRCLE_MIN_LINE_LEN 6
	...
	int maxsegnum = 0;
	for (int i=0; i < segmentNos+1; ++i){
		if (segmentStartLines[i] > maxsegnum) maxsegnum =  segmentStartLines[i] + 1;
	}

	// ------------------------------- DETECT ARCS ---------------------------------
	// std::cout << "detect arcs " << maxsegnum << ' ' << segmentNos << std::endl;

	info = new Info[maxsegnum];

	// Compute the angle information for each line segment
	for (int i = 0; i < segmentNos; i++) {

		// cout << "segment start lines " << segmentStartLines[i] << ' ' << segmentStartLines[i+1];
	...
	// std::cout << "enter detect arcs" << std::endl;
	DetectArcs(lines, maxsegnum);    // Detect all arcs
	// std::cout << "out of detect arcs" << std::endl;
	...
	//----------------------------- VALIDATE CIRCLES --------------------------
	// std::cout << "validate circles" << std::endl;
	...
	// std::cout << "join circles" << std::endl;
...
#define CIRCLE_MIN_LINE_LEN 6
	...
	edarcs1 = new EDArcs(maxNoOfCircles);
	DetectArcs(lines, 0);    // Detect all arcs
...
void EDCircles::DetectArcs(vector<LineSegment> lines, int maxsegnum)
{
		...
		for (int curSegmentNo = 0; curSegmentNo < segmentNos; curSegmentNo++) {
			...
			while (firstLine < stopLine - 1) {
				...
				while (firstLine <= lastLine - 2) {
					...
					firstLine = curLine;
					// std::cout << "P1" << double(curLine) << ' ' << double(maxsegnum) << std::endl;
					if (curLine < maxsegnum)
							{info[curLine].taken = true;}
...
void EDCircles::ValidateCircles()
{
	...
	for (int i = 0; i < noCircles1; ) {
		...
		validateAgain = false;
		//calculate nPoints
		// std::cout << "calculate npoints" << std::endl;
		int noPoints = 0;
		if (circle->isEllipse) {
			...
			CalculatenPoints(radius, &noPoints);
		} //end-else
		// std::cout << "nPOints" << noPoints << std::endl;


		double* px = new double[2*noPoints];			
		double* py = new double[2*noPoints];
		...
		for (int j = 0; j < noPoints; j++) {
			...
			int r = (int)(py[j] + 0.5);
			int c = (int)(px[j] + 0.5);
			// std::cout << "index" << double(r * width + c) << std::endl;
			// std::cout << edgeImg[r * width + c] << std::endl;
		...
		out:
			...
			// compute gx & gy
			// std::cout << "smooth image" << std::endl;
...
//
void EDCircles::CalculatenPoints(double r, int* noPoints){
